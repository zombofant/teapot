...
<test>
    ...
    def test_barrier(self):
        b = self._tree.tree.find("b")
        self.assertEqual(
            len(self._tree.tree.xpath("descendant::d",
                                      namespaces=self._tree.namespaces)),
            2)
        self.assertEqual(
            len(self._tree.tree.xpath("descendant::d["+
                                      self._tree.xpath_selector_non_barred+"]",
                                      namespaces=self._tree.namespaces)),
            2)
        self._tree.set_element_barrier(b)
        self.assertEqual(
            len(self._tree.tree.xpath("descendant::d["+
                                      self._tree.xpath_selector_non_barred+"]",
                                      namespaces=self._tree.namespaces)),
