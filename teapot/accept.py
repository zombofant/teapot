...
.. autoclass:: PreferenceList
   :members:

.. autoclass:: AcceptPreferenceList
   :members:
...
class Preference(object):
    """
    Represent a preference declaration according to RFC 2616, as used
    in the HTTP ``Accept-*`` headers. There exist specialized
    subclasses for each type of supported ``Accept`` header:
    ...
    .. note::
       ...
       It is recommended that you create instances of the above
       classes using the respective implementation of
       :meth:`~.from_header_section`. Do not create instances of
       *this* class—they're not particularily useful.

    They only differ in the way they implement the parsing method
    :meth:`~.from_header_section`.

    :class:`.Preference` instances order according to an internal key
    which is gives a stable ordering and is hashable.
    ...
    def __init__(self, value, q, parameters={}):
        ...
        self.rfc_key = (self.precedence,
                        self.q,
                        len(self.parameters))
        self.full_key = (self.precedence,
                         self.q,
                         self.value,
    ...
    def __repr__(self):
        return "<Preference {0!s};q={1:.2f}>".format(self, self.q)
    ...
    def match(self, other_pref, allow_wildcard=True):
        """
        Try to match *other_pref* against this match and return a
        tuple reflecting the quality of the match:
        ``(wildcard_penalty, keys_used, q)``.

        *wildcard_penalty* is a non-positive integer, which is equal
        to :attr:`~.precedence` if a wildcard match was successfully
        done, zero otherwise (i.e. also zero if no wildcard match was
        neccessary).

        *keys_used* is the number of keys from :attr:`parameters`
        which compared equal. This is always less or equal to
        ``len(parameters)``.

        *q* is the value of :attr:`~.q` of *this* instance or zero if
        the match failed.
        """
    ...
    def from_header_section(cls, value, drop_parameters=False):
        """
        This method is overriden in the subclasses with custom
        behaviour to correctly interpret the *value* string.

        *drop_parameters* indicates whether additional parameters will
        be dropped. These are normally passed to the *parameters*
        keyword argument of the constructor. The meaning of
        “parameters” is specific to the subclass.
...
class AcceptPreference(Preference):
    """
    Subclass of :class:`Preference` for dealing with ``Accept`` header
    values.
    """
    @classmethod
...
class PreferenceList(list):
    ...
    Objects of this class are iterable and respond to :func:`len`, but
    cannot be accessed element-wise, as that would not make any sense.

    .. warning::

       If you're dealing with ``Accept-Charset`` headers, please do
       not overlook the method
    ...
    def append_header(self, header, drop_parameters=False):
        """
        Create :class:`Preference` objects from the elements in the
        full HTTP header value *header* and add them to the
        list. *drop_parameters* is passed to the respective parser
        method.
        """
        if not header:
            return
    ...
    def get_candidates(self, own_preferences,
        ...
        Takes an iterable of :class:`Preference` instances
        *own_preferences* which indicate which preferences the
        application has (i.e. what to watch out for).

        This goes through the whole process specified in RFC 2616 and
        is fully tested to work. The process can be tweaked a little
        off-spec for certain special cases:

        * If *match_wildcard* is set to false, no wildcard matches are
          allowed.
        * If *include_non_matching* is set to true, all elements from
          this list which did not match the *own_preferences* are
          returned too, but ordered in front of others (i.e. with
          lower relative preference).
        * If *take_everything_on_empty* is set to False, an empty list
          is returned if no elements are in this list instead of
          returning the whole list of *own_preferences* converted
          into the return format specified above.

        .. note::

            The returned list is sorted for ascending match accuracies
            according to the RFC, **not** for *q* values. This implies
            that the last element of the list is the one you should
            pick if you can only serve the preferences given in
            *own_preferences* to comply with RFC 2616, see
            :meth:`best_match`.
        """
        if len(self) == 0:
    ...
    def get_quality(self, preference, match_wildcard=True):
        """
        Return the relative quality value for a given
        :class:`Preference` *preference* if you were about to return
        it.

        This goes through the whole matching process described in
        :meth:`get_candidates`, with *own_preferences* set to
        ``[preference]`` and returns the ``q`` value of the best
        match.
        """
    ...
    def best_match(self, own_preferences, match_wildcard=True):
        """
        Return the preference to use if you can deliver all
        preferences in *own_preferences* to give the user agent the
...
class CharsetPreferenceList(PreferenceList):
    """
    Subclass of :class:`PreferenceList` for HTTP ``Accept-Charset``
    headers.
    """
...
class LanguagePreferenceList(PreferenceList):
    """
    Subclass of :class:`PreferenceList` for HTTP ``Accept-Language``
    headers.
    """
